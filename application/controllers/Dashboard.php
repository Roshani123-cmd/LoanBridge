...
class Dashboard extends CI_Controller {
	...
	public function index()
	{
		...
		$allLoanType = $this->Dashboard_model->GetData("mst_loans", "", "", "created DESC", "", "");
		$userCond = '';
	...
	public function seenNotification(){
		...
		$getCount = 0;
		if(!empty($_POST['id'])){
			$updateNotice = $this->Dashboard_model->SaveData("notices",array('notice_seen'=>'Seen'),"id='".$_POST['id']."' and user_id=".$this->session->userdata('id')."");
			$getNoticeData = $this->Dashboard_model->GetData("notices", "", "notice_seen='Unseen' and user_id=".$this->session->userdata('id')."", "", "", "", "");

			$getCount = count($getNoticeData);
			$html.= '<h6 class="dropdown-header">
                    Alerts Center
                </h6>';
                 if(!empty($getNoticeData)) { 
                    foreach($getNoticeData as $data){
               $html.='<a class="dropdown-item d-flex align-items-center" href="javascript:void(0)"
                    onclick="openNoticePopUp(`'.$data->id.'`,`'.$data->title.'`,`'.$data->description.'`)">
					<div class="mr-3">
					...
					<div>
						<div class="small text-gray-500">'.date("F d, Y",strtotime($data->notice_date)).'</div>
					<div><span class="font-weight-bold">'.$data->title.'</span></div>
					<div><span>'.$data->description.'</span></div>
					</div>
					</a>';
	} 
}
	$data = array('success'=>1,'html'=>$html,'count'=>$getCount);
}else{
	$data = array('success'=>0,'html'=>$html,'count'=>$getCount);
}
print_r(json_encode($data));exit();
}
}
